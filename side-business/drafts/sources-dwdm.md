> **【2026-08-30 追記】採用ソースが1件確保できた。**
> **GNPy（Telecom Infra Project / BSD 3-Clause）** — PyPI 経由で取得可能。
> OSNR・ASE・分散・非線形の実装を含み、**有料の技術書に使える唯一のライセンス**。
> 検証内容は `research/07-network-access.md` §4〜5 を参照。
> 本文書の「採用0件」は Web ページに関するもので、パッケージレジストリ経由の経路は別途確保されている。
